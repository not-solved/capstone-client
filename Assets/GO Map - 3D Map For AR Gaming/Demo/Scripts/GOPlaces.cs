...
//			Debug.LogWarning ("Coordinates outside the tile");
		...
		public void ReloadMap(List<BombInformation> bomblist) {
			
			goMap.OnTileLoad.RemoveAllListeners();
			goMap.OnTileLoad.AddListener((GoTile) => {
				StartCoroutine(SearchAgain(GoTile, bomblist));
			});
			

		}
		IEnumerator SearchAgain (GOTile tile, List<BombInformation> list) {
			...
			if (string.IsNullOrEmpty (www.error)) {
				...
				for(int i = 0; i < list.Count; i++) {
					GameObject bombPlace = GameObject.Instantiate(prefab);
					Coordinates coord = new Coordinates(list[i].latitude, list[i].longitude, 0);
					Vector3 position = coord.convertCoordinateToVector(bombPlace.transform.position.y);
					bombPlace.transform.localPosition = position;
					bombPlace.transform.SetParent(placesContainer.transform);
					/*
					GameObject bombPlace = GameObject.Instantiate(prefab);
					Coordinates coord = new Coordinates(BombList[i].latitude, BombList[i].longitude, 0);
					Vector3 position = coord.convertCoordinateToVector(bombPlace.transform.position.y);
					bombPlace.transform.localPosition = position;
					bombPlace.transform.SetParent(placesContainer.transform);
					bombPlace.name = BombList[i].bombID + "_map";
					*/
				}
